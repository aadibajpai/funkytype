...
import { Music, RefreshCw, Trophy, Zap, Unlock, Clock, HelpCircle } from "lucide-react"
import { cn } from "@/lib/utils"
import Confetti from "./components/confetti"
import { verifyTypingTest, getTypingChallenge } from "./actions"
...
export default function Home() {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const estimatedAccuracy = estimateAccuracy(inputValue)
    setAccuracy(estimatedAccuracy)

    // Estimate errors
    setErrors(Math.round(((100 - estimatedAccuracy) * displayText.length) / 100))

    // Check if test is complete based on word count
    // Since we don't know the exact length of the complete text,
    // we'll use the display text length as an approximation
    if (inputValue.length >= displayText.length) {
      finishTest()
    }
  }
  ...
  const finishTest = () => {
    ...
    const finalWpm = Math.round(text.trim().split(/\s+/).length / ((endTimeValue - startTime) / 1000 / 60))
    setWpm(finalWpm)
      ...
      <div className="text-xl font-mono">
        {displayWords.map((displayWord, wordIndex) => {
          ...
          const wordStart = displayText.indexOf(
            displayWord,
            wordIndex > 0
              ? displayText.indexOf(displayWords[wordIndex - 1]) + displayWords[wordIndex - 1].length + 1
              : 0,
          )
            ...
            <span key={wordIndex} className="inline-block mr-2 mb-2">
              {displayWord.split("").map((displayChar, charIndex) => {
                ...
                if (absoluteIndex < text.length) {
                  ...
                  state = displayChar !== "_" && text[absoluteIndex] === displayChar ? "correct" : "incorrect"
                }
                ...
                return (
                  <span
                    ...
                    className={cn(
                      "transition-colors",
                      isMissingLetter ? "border-b-2 border-dashed border-white/50" : "",
                      isMissingLetter
                        ? "text-white/40"
                        : // Make underscores more subtle
                          ...
                          : state === "incorrect"
                            ? "text-red-400"
                            : "text-white/80",
                      isActive && "bg-white/20 border-b-2 border-white animate-pulse",
                    )}
                  >
                    {displayChar}
                  </span>
  ...
  return (
    <main
      className={cn("min-h-screen flex flex-col items-center justify-center p-4 transition-colors", getThemeClass())}
    >
      {showConfetti && <Confetti />}

      <div className="w-full max-w-3xl flex flex-col items-center gap-6" key="funkytype-main-container">
        <div className="flex items-center gap-2 mb-2">
          <Zap className="h-8 w-8 text-white" />
          <h1 className="text-4xl font-bold text-white tracking-tight">FunkyType</h1>
        </div>
          ...
          <CardContent className="p-6">
            ...
            {showHelp && (
              <div className="mb-4 p-3 bg-black/30 rounded-lg text-white/90 text-sm">
                ...
                <p>Some letters are missing (shown as "_"). You must guess and type the complete words correctly.</p>
              </div>
            )}
            ...
            <div className="relative mb-4">
              {isLoading ? (
                ...
                <div ref={textDisplayRef} className="h-64 overflow-y-auto p-4 bg-black/20 rounded-lg leading-relaxed">
                  {renderText()}
                </div>
              )}
            ...
            <Progress value={(text.length / displayText.length) * 100} className="h-2 bg-white/20" />
              ...
              <div className="mt-6 p-4 bg-white/10 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-white text-2xl font-bold">{((endTime - startTime) / 1000).toFixed(1)}s</p>
                  </div>
                </div>

                {wpm >= 500 ? (
                  <div className="mt-4 p-3 bg-gradient-to-r from-yellow-400 to-orange-500 rounded-lg text-center">
                    <p className="text-black font-bold">🎉 AMAZING! You reached 500+ WPM! 🎉</p>
                  </div>
                ) : (
                  <div className="mt-4 p-3 bg-white/10 rounded-lg text-center">
                    <p className="text-white/80">Keep practicing! You need {500 - wpm} more WPM to reach the goal.</p>
                  </div>
                )}
                ...
                {isSubmitting ? (
                  ...
                  <div className="mt-4 p-4 bg-red-500/20 border border-red-500/50 rounded-lg">
                    <p className="text-red-300">{errorMessage}</p>
                    <Button onClick={resetTest} className="w-full mt-4 bg-white/20 hover:bg-white/30 text-white">
                      Try Again
                    </Button>
                  </div>
                ) : flag ? (
                  <div className="mt-4 p-4 bg-green-500/20 border border-green-500/50 rounded-lg">
                    ...
                    <p className="font-mono bg-black/30 p-2 rounded text-green-400 break-all">{flag}</p>
                  </div>
                ) : (
                  <div className="mt-4 p-3 bg-white/10 rounded-lg text-center">
                    <Button onClick={resetTest} className="w-full bg-white/20 hover:bg-white/30 text-white">
                      Try Again
                    </Button>
                  </div>
                )}
        ...
        <Tabs defaultValue="theme" className="w-full max-w-md">
          <TabsList className="grid grid-cols-1 bg-black/40 backdrop-blur-md border-white/20">
            <TabsTrigger value="theme" className="text-white data-[state=active]:bg-white/20">
              <Music className="h-4 w-4 mr-2" /> Theme
            </TabsTrigger>
          </TabsList>
          <TabsContent value="theme" className="bg-black/40 backdrop-blur-md border-white/20 p-4 rounded-md mt-2">
        ...
        <p className="text-white/80 text-center text-sm">Challenge: Type at 500+ WPM to unlock the flag! 🚀</p>
      </div>
    </main>
  )
}
